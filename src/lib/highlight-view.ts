...
/**
 * How far to nudge the scroll after the library's top-anchored
 * scrollToHighlight so the highlight lands where the waypoint asked for.
 *
 * "top" is where the library already leaves it, so it needs no nudge. Both of
 * the others fall back to that when the highlight is taller than the viewport,
 * where moving it down would push its opening lines off-screen.
 */
export function alignScrollOffset(
  align: ScrollAlign,
  highlightHeight: number,
  containerHeight: number,
  topMargin: number = LIB_SCROLL_MARGIN,
  ...
  if (highlightHeight >= containerHeight - 2 * topMargin) return 0;
  if (align === "bottom") {
    return 2 * topMargin + highlightHeight - containerHeight;
  }
  return centeringScrollOffset(highlightHeight, containerHeight, topMargin);
}
